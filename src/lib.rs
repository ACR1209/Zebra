...
impl Matrix {
    ...
    pub fn matrix_from_str(s: &str) -> Matrix {
    ...
    fn valid_matrix_contents(data: &[Vec<f64>]) -> bool {
    ...
    pub fn identity(&mut self) {
        if !self.is_square() {
            panic!(
                "{}",
                Error::new(
                    "Not a squared matrix.".to_string(),
                    "DimensionError".to_string()
                )
            );
        }
    ...
    pub fn size(&self) -> usize {
        std::cmp::max(self.cols, self.rows)
    }
